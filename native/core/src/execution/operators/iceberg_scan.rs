...
impl IcebergScanExec {
    fn execute_with_tasks(
        ...
        // Create file stream that uses iceberg-rust's file fetching
...
/// Stream that reads Iceberg files using iceberg-rust's parallel file fetching.
///
/// Delegates to iceberg-rust's ArrowReader which uses `try_buffer_unordered` to
/// fetch multiple files concurrently, controlled by `concurrency_limit_data_files`.
    ...
    fn new(
        ...
        let adapted_stream = arrow_reader_stream
            ...
            .and_then(move |batch| {
                ...
                let adapter =
                    adapter_factory.create(Arc::clone(&target_schema), Arc::clone(&file_schema));
...
impl Stream for IcebergFileStream {
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        let result = self.metrics.baseline.record_poll(poll_result);
